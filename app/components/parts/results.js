            ...
            <tr key={index}>
              <td>
                <table>
                  <tbody>
                  <tr>
                    <td>{isRebuildKit ? <span className={classNames('rebuild-kit-toggle', { show: this.state.rebuildKitActive })} onClick={() => this.setState({ rebuildKitActive: !this.state.rebuildKitActive })} /> : null}{partName}</td>
                    <td className="center">
                      {part.PartNumber}
